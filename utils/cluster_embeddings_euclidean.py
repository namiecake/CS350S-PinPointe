# made in china ... oops i mean made in claude

#!/usr/bin/env python3
"""
Cluster user profile embeddings using MiniBatchKMeans.
...
def perform_dimensionality_reduction(sparse_matrix, n_components=300, random_state=42):
    """
    Apply TruncatedSVD for dimensionality reduction.
    
    Args:
        sparse_matrix: Input sparse matrix
        n_components: Number of components to keep (default: 300)
...
def perform_clustering(matrix, n_clusters, random_state=42):
    """Perform MiniBatchKMeans clustering on matrix (can be sparse or dense)."""
    print(f"\nPerforming MiniBatchKMeans clustering with {n_clusters} clusters...")
...
def assign_multiple_clusters(matrix, kmeans, user_ids, top_k=3, distance_threshold=None):
    """
    Assign users to multiple clusters based on proximity to cluster centers.
    
    Args:
        matrix: User embedding matrix (sparse or dense)
...
def compute_silhouette_score_sample(matrix, labels, sample_size=5000):
    ...
    if len(labels) > sample_size:
        ...
        score = silhouette_score(sample_matrix, sample_labels, metric='euclidean')
    else:
...
def save_clustering_results(user_ids, labels, kmeans, cluster_sizes, output_path, 
    ...
    if svd_model is not None:
        ...
        output_data['metadata']['centroid_space'] = 'reduced_svd'
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Cluster user embeddings using dimensionality reduction + MiniBatchKMeans'
    )
    ...
    output_path = data_dir / 'user_clusters_euclidean.json'
    ...
    kmeans, labels = perform_clustering(reduced_matrix, n_clusters)
    
    # Compute and display statistics
    cluster_sizes = compute_cluster_statistics(labels, n_clusters)
    
    # Compute silhouette score (on sample if dataset is large)
    compute_silhouette_score_sample(reduced_matrix, labels, sample_size=5000)
    ...
    if args.multi_cluster:
        ...
        multi_assignments = assign_multiple_clusters(
            reduced_matrix, kmeans, user_ids, 
            top_k=args.top_k,
