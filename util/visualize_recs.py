...
def main():
    ...
    book_index_to_title = load_json('book_index_to_title.json')['book_index_to_title']
    recs_per_cluster = load_json('server/recs_per_cluster_implicit.json')['cluster_recommendations']
    ...
    if cluster_id not in recs_per_cluster:
        print(f"Error: Cluster {cluster_id} not found in recs_per_cluster.json")
        return
    ...
    for i, user_id in enumerate(selected_users, 1):
        ...
        top_books = get_user_top_books(user_ratings, n=5)
    ...
    recs = recs_per_cluster[cluster_id][:20]
    
    for i, book_index in enumerate(recs, 1):
        title = book_index_to_title.get(str(book_index), f"Unknown (Index: {book_index})")
        print(f"{i:2d}. {title}")
